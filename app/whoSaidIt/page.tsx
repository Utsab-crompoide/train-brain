  ...
  return (
    <div className="flex gap-1.5 justify-center flex-wrap">
      {Array.from({ length: total }).map((_, i) => (
        <div key={i} className={`h-2 w-5 rounded-full transition-all duration-300 ${i < current ? "bg-indigo-500" : i === current ? "bg-indigo-400/60" : isDark ? "bg-gray-700" : "bg-gray-200"}`} />
      ))}
    </div>
  );
}
...
function OptionCard({ text, state, isDark, onClick }: { text: string; state: OptionState; isDark: boolean; onClick: () => void }) {
  const isInteractive = state === "idle";

  const containerClass: Record<OptionState, string> = {
    idle: isDark ? "border-gray-700 bg-gray-900/40 hover:bg-gray-800/60 hover:border-gray-600 cursor-pointer active:scale-[0.98]" : "border-gray-200 bg-gray-50/60 hover:bg-white hover:border-gray-300 shadow-sm cursor-pointer active:scale-[0.98]",
    correct: isDark ? "border-emerald-600 bg-emerald-900/40 cursor-default" : "border-emerald-400 bg-emerald-50 cursor-default",
    wrong: isDark ? "border-red-700 bg-red-900/40 cursor-default" : "border-red-400 bg-red-50 cursor-default",
    disabled: isDark ? "border-gray-800 bg-gray-900/20 opacity-40 cursor-default" : "border-gray-300 bg-gray-100 opacity-60 cursor-default",
  };
  ...
  return (
    <div className={`flex items-center justify-between rounded-xl px-5 py-4 border transition-all duration-300 ${containerClass[state]}`} onClick={isInteractive ? onClick : undefined}>
      <span className={`text-sm font-semibold leading-snug transition-colors duration-300 ${textClass[state]}`}>{text}</span>
      {icon[state] && <span className={`flex-shrink-0 w-6 h-6 flex items-center justify-center rounded-full text-xs font-bold ml-3 ${state === "correct" ? "bg-emerald-600 text-white" : "bg-red-600 text-white"}`}>{icon[state]}</span>}
    </div>
  );
}
  ...
  return (
    <div className={`flex min-h-screen flex-col items-center font-sans transition-colors duration-300 ${isDark ? "bg-gray-950" : "bg-gray-50"}`}>
      ...
      <main className="flex w-full max-w-lg flex-col px-5 py-12 sm:px-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className={`text-4xl font-extrabold tracking-tight sm:text-5xl ${isDark ? "text-white" : "text-gray-900"}`}>Who Said It?</h1>
          <p className={`mt-2 text-sm ${isDark ? "text-gray-500" : "text-gray-400"}`}>Match the quote to the person who said it.</p>
        </div>
        ...
        {/* Quote card */}
        <div className={`mb-5 rounded-2xl px-6 py-5 border flex flex-col gap-3 ${isDark ? "bg-gray-900/60 border-gray-800" : "bg-white border-gray-200 shadow-sm"}`}>
          <div className="flex items-center justify-between">
            <span className={`text-xs uppercase tracking-widest ${isDark ? "text-gray-500" : "text-gray-400"}`}>{game.puzzle.category}</span>
            <DifficultyBadge difficulty={game.puzzle.difficulty} isDark={isDark} />
          </div>
          <p className={`text-base font-semibold leading-relaxed ${isDark ? "text-gray-100" : "text-gray-900"}`}>"{game.puzzle.quote}"</p>
        </div>

        {/* Options */}
        <div className={`flex flex-col gap-3 mb-5 ${game.shake ? "animate-shake" : ""}`}>
          {game.shuffledOptions.map((option) => (
            <OptionCard key={option} text={option} state={getOptionState(option)} isDark={isDark} onClick={() => game.select(option)} />
          ))}
        </div>
