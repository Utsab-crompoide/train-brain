...
function DifficultyBadge({ difficulty, isDark }: { difficulty: string; isDark: boolean }) {
  ...
  return (
    <span className={`inline-block text-xs font-semibold uppercase tracking-widest px-2.5 py-1 rounded-lg border ${colors[difficulty]}`}>
      {difficulty}
    </span>
  );
}
...
function StatementCard({
  ...
  const baseClass =
    "flex items-start gap-4 rounded-xl px-5 py-4 border transition-all duration-400 cursor-pointer select-none";

  const stateClass: Record<CardState, string> = {
    idle: isDark
      ? "border-gray-700 bg-gray-900/40 hover:bg-gray-800/60 hover:border-gray-600 active:scale-[0.98]"
      : "border-gray-200 bg-gray-50/60 hover:bg-white hover:border-gray-300 shadow-sm active:scale-[0.98]",
    "selected-correct": isDark
      ? "border-emerald-600 bg-emerald-900/40 cursor-default"
      : "border-emerald-400 bg-emerald-50 cursor-default",
    "selected-wrong": isDark
      ? "border-red-700 bg-red-900/40 cursor-default"
      : "border-red-400 bg-red-50 cursor-default",
    "revealed-true": isDark
      ? "border-gray-800 bg-gray-900/20 opacity-50 cursor-default"
      : "border-gray-100 bg-gray-50/30 opacity-50 cursor-default",
    "hint-safe": isDark
      ? "border-indigo-700 bg-indigo-900/30 cursor-default"
  ...
  return (
    ...
    >
      {/* Label */}
      <span
        className={`flex-shrink-0 w-7 h-7 flex items-center justify-center rounded-lg border text-xs font-bold transition-all duration-300 ${labelClass[state]}`}
      >
        {icon[state] ?? label}
      </span>

      {/* Text */}
      <p className={`text-sm leading-relaxed font-medium transition-colors duration-300 ${textClass[state]}`}>
        {text}
      </p>
    </div>
  );
...
function StreakBadge({ streak, isDark }: { streak: number; isDark: boolean }) {
  ...
  return (
    <div className={`flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full border ${isDark ? "bg-orange-900/40 border-orange-800 text-orange-400" : "bg-orange-50 border-orange-200 text-orange-600"}`}>
      🔥 {streak} streak
    </div>
  );
}
  ...
  return (
    ...
    >
      ...
      <main className="flex w-full max-w-lg flex-col px-5 py-12 sm:px-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className={`text-4xl font-extrabold tracking-tight sm:text-5xl ${isDark ? "text-white" : "text-gray-900"}`}>
            The Liar
          </h1>
          <p className={`mt-2 text-sm ${isDark ? "text-gray-500" : "text-gray-400"}`}>
            Two truths, one lie. Spot the liar.
          </p>
        </div>
        ...
        <div className="flex items-center justify-between mb-6">
          <p className={`text-sm font-semibold ${isDark ? "text-gray-400" : "text-gray-500"}`}>
            Score:{" "}
            <span className={`text-base font-extrabold ${isDark ? "text-white" : "text-gray-900"}`}>
              {game.score}
            </span>
          </p>
        ...
        {/* Topic card */}
        <div
          className={`mb-5 rounded-2xl px-5 py-4 border flex items-center justify-between ${
            isDark
        ...
        >
          <div>
            <p className={`text-xs uppercase tracking-widest mb-1 ${isDark ? "text-gray-500" : "text-gray-400"}`}>
              Topic
            </p>
        ...
        <p className={`mt-8 text-center text-xs ${isDark ? "text-gray-700" : "text-gray-400"}`}>
